...
var mapid = new Array(350060170, 350060190, 350060210);
var monster = new Array(8240103, 8240104, 8240105);
...
function setup(eim) {
    ...
    while (em.getInstance("HardSwoo" + a) != null) {
        a = Randomizer.nextInt();
    }
    var eim = em.newInstance("HardSwoo" + a);
    return eim;
}
...
function monsterKilled(eim, player, mobid) {
    if (mobid == monster[Integer.parseInt(eim.getProperty("Stage"))] && mobid != 8240105) {
        eim.restartEventTimer(5000);
        eim.setProperty("nextWarp", "true");
    } else if (mobid == 8240105) {
       WorldBroadcasting.broadcast(MainPacketCreator.serverNotice(6, "블랙해븐의 스우를 물리친 원정대여! 그대들이 진정한 영웅이다!"));
        var it = eim.getPlayers().iterator();
        while (it.hasNext()) {
            var chr = it.next();
            eim.unregisterAll();
