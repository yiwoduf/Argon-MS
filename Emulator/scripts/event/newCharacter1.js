...
function setup(eim) {
    var eim = em.newInstance("newCharacter1");
...
function playerEntry(eim, player) {
    ...
    var mapzz = eim.getMapFactory().getMap(931050810);
    em.changeMap(mapzz, mapzz.getPortal("sp"));
}

function announce(eim) {
    var text = "3초 후 보스몬스터가 소환됩니다.";
    eim.getMapFactory().getMap(931050810).broadcastMessage(UIPacket.showInfo(text));
    eim.getMapFactory().getMap(931050810).broadcastMessage(MainPacketCreator.getGMText(7, text));
...
function summon(eim) {
    ...
    eim.getMapFactory().getMap(931050810).broadcastMessage(UIPacket.showInfo(text));
    eim.getMapFactory().getMap(931050810).broadcastMessage(MainPacketCreator.getGMText(5, text)); //-109,154
    var mobid = 0;
    var level = Integer.parseInt(eim.getProperty("level"));
    if (level == 0) {
        mobid = 8180000;
    ...
    for (var i = 0 ; i < 1 ; i++) {
	    ...
	    modified.setOHp(mob.getMobMaxHp() * 17);
	    modified.setOMp(mob.getMobMaxMp() * 1.5);
	    mob.setOverrideStats(modified);
        eim.getMapFactory().getMap(931050810).spawnMonsterOnGroundBelow(mob, new Point(-2090, 29));
    }
    level++;
...
function allMonstersDead(eim) {
    ...
    if (level >= 1 && level <= 4) {
        ...
        eim.getMapFactory().getMap(931050810).broadcastMessage(UIPacket.showInfo(text));
        eim.getMapFactory().getMap(931050810).broadcastMessage(MainPacketCreator.showEffect("monsterPark/clear"));
        eim.getMapFactory().getMap(931050810).broadcastMessage(MainPacketCreator.playSound("Party1/Clear"));
        eim.getMapFactory().getMap(931050810).broadcastMessage(MainPacketCreator.serverNotice(6, text));
    ...
    if (level == 5) {
        ...
        eim.getMapFactory().getMap(931050810).broadcastMessage(UIPacket.showInfo(text));
        eim.getMapFactory().getMap(931050810).broadcastMessage(MainPacketCreator.getClock(7));
        eim.getMapFactory().getMap(931050810).broadcastMessage(MainPacketCreator.serverNotice(6, text));
        //eim.getMapFactory().getMap(931050810).broadcastMessage(MainPacketCreator.serverNotice(6, "미스틱필드 클리어 보상으로 판테온 미스틱 문서 1장이 지급되었습니다."));
        var chr = eim.getPlayers().iterator().next();
