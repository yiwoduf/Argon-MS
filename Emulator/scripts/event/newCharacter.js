...
function summon(eim) {
    ...
    for (var i = 0 ; i < 1 ; i++) {
	    ...
	    mob.setOverrideStats(modified);
            eim.getMapFactory().getMap(931050800).spawnMonsterOnGroundMaple(mob, new Point(3050, 575));
    }
    level++;
...
function allMonstersDead(eim) {
    ...
    if (level == 5) {
        ...
        eim.getMapFactory().getMap(931050800).broadcastMessage(MainPacketCreator.serverNotice(6, text));
        //eim.getMapFactory().getMap(931050800).broadcastMessage(MainPacketCreator.serverNotice(6, "미스틱필드 클리어 보상으로 헤네시스 미스틱 문서 1장이 지급되었습니다."));
        var chr = eim.getPlayers().iterator().next();
...
function clearPQ(eim) {
    ...
    var chr = eim.getPlayers().iterator().next();
    eim.removePlayer(chr);
    //chr.gainItem(4031611, 1, false, -1, "헤네시스 미스틱 문서");
