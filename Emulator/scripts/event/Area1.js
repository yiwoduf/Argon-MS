...
function playerEntry(eim, player) {
    player.send(UIPacket.showInfo("드래고니카 : 여기에 들어온 무례한자는 누구냐!"));
    player.send(MainPacketCreator.getGMText(7, "드래고니카 : 여기에 들어온 무례한자는 누구냐!"));
...
function summon(eim) {
    var mobid = 0;
    var level = Integer.parseInt(eim.getProperty("level"));
    eim.getMapFactory().getMap(240080500).broadcastMessage(UIPacket.showWZEffect("UI/tutorial.img/20", 1));
    if (level == 0) {
        mobid = 8300006;
    } else if (level == 1) {
        mobid = 8300007;
    ...
    eim.getMapFactory().getMap(240080500).spawnMonsterOnGroundBelow(mob, new java.awt.Point(17, -10));
}
    if (level == 1){ 
    var mob = MapleLifeProvider.getMonster(mobid);
    eim.registerMonster(mob);
...
function allMonstersDead(eim) {
    ...
    if (level == 2) {
        var text = "드래고니카가 봉인되었습니다.";
        var chr = eim.getPlayers().iterator().next();
        var exit = em.getChannelServer().getMapFactory().getMap(100000000);
        eim.getMapFactory().getMap(240080500).broadcastMessage(UIPacket.showInfo(text));
        eim.getMapFactory().getMap(240080500).broadcastMessage(MainPacketCreator.getClock(20));
        eim.getMapFactory().getMap(240080500).broadcastMessage(MainPacketCreator.showEffect("monsterPark/clearF"));
        eim.getMapFactory().getMap(240080500).broadcastMessage(MainPacketCreator.playSound("Party1/Clear"));
        eim.getMapFactory().getMap(240080500).broadcastMessage(MainPacketCreator.serverNotice(6, text));
