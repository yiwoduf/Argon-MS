...
function playerEntry(eim, player) {
    ...
    player.message(5, "15분 내에 자쿰을 소환하지 않으면 자쿰의 제단이 닫힙니다.");
...
function monsterValue(eim, mobid)
{
	if(mobid == 8800002)
	{
		...
		{
			var chr = it.next();
			chr.Message(8, ""+chr.getName()+" 님께서 참여하신 자쿰가(이) "+inz[1]+"월 "+inz[2]+"일 "+inz[3]+"시 "+inz[4]+"분 클리어 되어 기록이 저장되었습니다.");
	...
	else if(mobid == 8800022)
	{
		...
		{
			var chr = it.next();
		chr.Message(8, ""+chr.getName()+" 님께서 참여하신 약화된 자쿰가(이) "+inz[1]+"월 "+inz[2]+"일 "+inz[3]+"시 "+inz[4]+"분 클리어 되어 기록이 저장되었습니다.");
	...
	else if(mobid == 8800102)
	{
		...
		{
			var chr = it.next();
		chr.Message(8, ""+chr.getName()+" 님께서 참여하신 카오스 자쿰가(이) "+inz[1]+"월 "+inz[2]+"일 "+inz[3]+"시 "+inz[4]+"분 클리어 되어 기록이 저장되었습니다.");
