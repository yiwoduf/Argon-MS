...
importPackage(Packages.main.world);
	...
	if(Status == 0) {
	cm.sendYesNo("메소꾸러기(소)를 열어보겠어? #b50만 메소에서 100억 메소#k 사이의 금액을 획득할 수 있어 너의 운을한번 시험해보겠니?"
		+"")
	}
		...
		cm.sendOk("#i2432393# #b#z2432393##k이 있다고 우겨서 되는게 아니야. 봐, 네 인벤토리에는 없잖아?");
		cm.dispose();
		} else {
		...
		cm.sendOk("축하해 메소 럭키백에서 "+W+"메소를 얻었어. 지금 바로 인벤토리를 확인해 봐.#b"
			...
			WorldBroadcasting.broadcast(MainPacketCreator.getGMText(20, "[대박] "+cm.getPlayer().getName()+" 님이 메소 꾸러기(소)에서 "+W+" 메소를 획득했습니다. 모두 축하해주세요~"));
			cm.gainItem(N, Q);
			} else {
			cm.gainMeso(N);
			}
