...
function action(mode, type, selection) {
    if (mode < 0)
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            cm.sendYesNo("#e<혼란에 빠진 미스틱 필드를 구원해주세요!>#n\r\n\r\n#e#bNo.2#n#k #fUI/UIWindow2.img/MobGage/Mob/9300594# #e#b미스틱 반레온#n#k  획득아이템 #i4033311# (1~2개))\r\n\r\n#e#r이동하시겠습니까?\r\n[파티를구성하지 않으면 입장이 불가능합니다.]\r\n#e#b#n#k#n#k\r\n");
        } else if (status == 1) {
            if (cm.getPlayer().getParty() != null) {
                if (cm.getPlayerCount(931050810) > 0) {
                    ...
                    cm.spawnMob(9300594, cm.getPlayer().getPosition().x + 800, cm.getPlayer().getPosition().y);
                    cm.dispose();
                }
            } else {
