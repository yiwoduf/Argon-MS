...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            var chat = "#b<보스 한꺼번에 2마리 이상 소환시 기존 몹은 제거됩니다.>#k \r\n#r<아카이럼 소환>#k : #i4001431# 1장#e#n";
            chat += "\r\n#r소지중인 #i4001431#갯수 :: "+ cm.itemQuantity(4001431) +"#k";
            chat += "\r\n#L0##b아카이럼를 소환 하고싶습니다.#k";
            chat += "\r\n#L1##b소환되어 있는 몹을 죽이고 싶습니다.(단, 아이템 드롭X)#k";
            chat += "\r\n#L2##b[AURORA STORY] 마을로 돌아가고싶습니다.#k";
            cm.sendSimple(chat);
        } else if (status == 1) {
            if (selection == 0) {
         if (cm.itemQuantity(4001431) >= 1  && cm.getMonsterCount(cm.getMapId()) == 0) {
        var chat = "보스 한꺼번에 2마리 이상 소환시 기존 몹은 제거됩니다.";
	cm.removeNpc(cm.getMapId(), 2083002);
        cm.gainItem(4001431 ,-1);
	cm.spawnMob(8860000, cm.getPlayer().getPosition().x + 300, cm.getPlayer().getPosition().y);
        cm.sendNext(chat);
	cm.dispose();
        }
     else {
