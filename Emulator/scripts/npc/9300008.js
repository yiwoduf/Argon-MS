...
function action(mode, type, selection) {
	...
	level = level(eq);
		...
		if(sel.equals("올스텟")) {
			...
			cm.fakeRelog();
			cm.updateChar();
			cm.sendOk("강화에 성공하여 올스텟 "+amount+" 이 추가되고 "+level+" 이 되었습니다.섭첸부탁드립니다."); 
			cm.dispose(); return;
		} else {
			...
			cm.fakeRelog();
			cm.updateChar();
			cm.sendOk("강화에 성공하여 공마 "+amount+" 이 추가되고 "+level+" 이 되었습니다. 섭첸부탁드립니다."); 
			cm.dispose(); return;
		}
    }
}
...
var chance;
	if(eq.getOwner().equals("1★")) {
 		chance = 70; // 1강에서 2강갈때 확률 ex) 10 : 10%
	} else if(eq.getOwner().equals("2★")) {
		chance = 60; // 2강에서 3강갈때 확률 ex) 10 : 10%
	} else if(eq.getOwner().equals("3★")) {
		chance = 50; // 3강에서 4강갈때 확률 ex) 10 : 10%
	} else if(eq.getOwner().equals("4★")) {
		chance = 40; // 4강에서 5강갈때 확률 ex) 10 : 10%
	} else if(eq.getOwner().equals("5★")) {
		chance = 30; // 5강에서 6강갈때 확률 ex) 10 : 10%
	} else if(eq.getOwner().equals("6★")) {
		chance = 15; // 6강에서 7강갈때 확률 ex) 10 : 10%
	} else {
...
function stat(eq, sel) {
	...
	if(eq.getOwner().equals("1★")) { // 1강 - 2강 넘어갈때 스텟
		stat = sel == "올스텟" ? Randomizer.rand(8, 50) : Randomizer.rand(5, 25);
						//	↑↑↑ 이것들 처럼 밑에거도 동일하게
