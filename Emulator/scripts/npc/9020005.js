...
function action(mode, type, selection) {
	...
	var chat = "미니게임 아이템을 만들고 싶다고? 미니게임은 그냥 할 수 있는게 아니야. 어떤 미니게임을 하고 싶다면 3,000,000메소가 필요하지. 어떤 미니게임 아이템을 만들고 싶지?#b\r\n";
	chat += "#L0#오목세트#l\r\n";
	chat += "#L1#같은그림찾기세트#l";
	cm.sendSimple(chat);
	oldsel = 1;
		...
		oldsel = 3;
		var chat = "#b#z4080100##k를 만들고 싶다는 말이지? 흐음… #z4080100#를 만들기 위해서는 3,000,000메소가 필요해.";
		...
		if(oldsel == 1) {
		var chat = "#b#z"+omok[selection][0]+"##k를 만들고 싶다는 말이지? 흐음… 3,000,000메소만 있으면 내가 만들어 줄 수 도있지";
		cm.sendNextPrev(chat);

		} else if (oldsel == 3) {
		if(cm.getMeso() >= 3000000) {
		cm.sendNextPrev("오랜만에 능력발휘 해볼까? 조금만 기달려 달라고~ #r#z4080100##k를 금방 만들어 줄테니 말야. 웃차 웃차~");
		oldsel = 4;
		} else {
		cm.sendOk("뭐야. 5,000,000메소가 없으면 아무리 뛰어난 미니게임 마스터인 나라도 만들어 줄 수 없다고.");
		cm.dispose();
	...
	cm.sendNextPrev("아참… 같은그림찾기는 오목과는 달리 방을 개설할 때 몇 장의 카드로 할 것인지 설정하는 것이 추가적으로 필요해. 3×4, 4×5, 5×6 이렇게 3가지 모드가 있어서 각각 12, 20, 30장의 카드로 같은 그림 찾기를 할 수 있지. 다만 이건 한 번 방이 열리면 도중에 바꿀 수 없으니까 바꾸고 싶으면 방을 닫았다가 다시 개설하는 수 밖에 없어.");
		...
		if(cm.getMeso() >= 3000000) {
		cm.sendNextPrev("좋았어. 조금만 기달려 달라고~ 금방 만들어 줄테니 말야. 웃차 웃차~");
			} else {
		cm.sendNextPrev("5,000,000메소가 없으면 아무리 뛰어난 미니게임 마스터인 나라도 만들어 줄 수 없다고.");
		cm.dispose();
		}	
	}
