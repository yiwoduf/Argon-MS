...
function action(mode, type, selection) {
 status++;
 if (status == 0) {
  cm.sendSimple("보스를 소환할 수 있습니다.\r\n#L1##i"+item+"# #r"+count+"개#k로 하드 매그너스를 소환 [피 1000조]#l\r\n#L3##i"+item+"# #r"+count2+"개#k로 하드 스우를 소환 [피 2500조] #l#k\r\n#L2#광장으로 가기#l#k\r\n");
 } else if(status == 1) {
  if(selection == 1) {
 if (cm.getMonsterCount(933037000) > 0) {
  ...
  cm.dispose();
			...
			cm.sendOk("하드 매그너스를 소환하려면 #i"+item+"#"+count+"개가 필요합니다.");
			cm.dispose();
 } else {    
WorldBroadcasting.broadcast(MainPacketCreator.serverNotice(6, "[SMART] ["+ cm.getPlayer().getName()+"] 님께서 "+(cm.getClient().getChannel()+1) +" 채널에서 하드매그너스 를 소환하셨습니다. [SMART]")); 
