...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			var ask = " #fn나눔고딕 Extrabold# 어느 타입의 아이템을 선물하시겠습니까?\r\n";
			...
			if (selection >= 1 && selection <=5) {
				cm.sendGetText("#fn나눔고딕 Extrabold# 선물받으실 분의 아이디를 입력해주세요.");
			} else if (selection == 6) {
				cm.sendOk("#fn나눔고딕 Extrabold# 선물시스템은 아이템의 종류나 옵션에 관계없이 누구에게나 선물할수 있는 시스템입니다. 선물을 하기 위해선 천만메소가 필요하며, 같은 채널에 접속중이여야 합니다. 잘못된 사용으로 인한 문제는 운영진 측에서 책임지지 않습니다.");
				cm.dispose();
			}
		} else if (status == 2) {
			if (operation == 1) {
		...
		if (conn == null){
		cm.sendOk("#fn나눔고딕 Extrabold# 현재 접속중이 아니거나 채널이 다릅니다. 혹은 존재하지 않는 아이디일 수도 있습니다.");
		cm.dispose();
		}else{
		var ok = false;
		...
		if (!ok) {
			cm.sendOk("#fn나눔고딕 Extrabold# 선물할 아이템이 없는것 같은데요?");
			cm.dispose();
			return;
		}
		...
		sel = selection;
			...
			selected = selection % 1000;
				...
				if (statsSel == null) {
					cm.sendOk("#fn나눔고딕 Extrabold# 오류입니다. 운영자에게 보고해주세요.");
					cm.dispose();
					return;
				}
			var text = cm.getText();
	...
	if (item.getQuantity() == 1){
		if (cm.getMeso()>=1000000){
			...
			cm.dispose();
				...
				cm.sendOk("#fn나눔고딕 Extrabold# 자기 자신에게는 선물할수 없습니다.");
				cm.dispose();
			}
			}else {
				cm.sendOk("#fn나눔고딕 Extrabold# 펫은 선물할수 없습니다.");
				cm.dispose();
			}
			}else{
			cm.sendOk("아이템을 선물하기 위해선 수수료 1천만메소가 필요합니다.");
			cm.dispose();
			}
			}else {
				cm.sendGetNumber("#fn나눔고딕 Extrabold# 몇개를 선물하시겠습니까?\r\n현재 소지중인 #i"+item.getItemId()+"# #b(#t"+item.getItemId()+"#)#k 갯수 : #b"+item.getQuantity()+"#k", 1, 1, item.getQuantity());
			}
			name = text;
		}else if (status==4){
