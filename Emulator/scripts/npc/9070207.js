...
function action(mode, type, selection) {
         ...
         if (status == 0) {
	      cm.sendGetText("#fn나눔고딕 Extrabold##fs14##e캐시아이템 이름을 입력하세요(정확한 이름을 입력 안할시 검색 내용이 너무 많아 팅길수도 있습니다.)#k\r\n\r\n추가옵션 : #g올스텟+0 공/마+0#k");
         } else if (status == 1) {
	  ...
	  if (cm.getPlayer().haveItem(4001126, 0, true, true)) { // 이하소지시 아이템구매 불가 

	      cm.sendOk("#i"+selection+"# #fs14##e#b#t"+selection+"##n#k#fs12#을(를) 획득하셨습니다.");
              cm.gainSponserItem(selection,'',0,0,0); // 올스텟,공격력
	      cm.gainItem(4001126, -0); // 삭감 갯수		
              cm.dispose();
	 } else {
	      cm.sendOk("#i4001126# #z4001126# 150개 미만이므로 아이템 구매가 불가능 합니다.");
              cm.dispose();
	}
    }
}