...
function action(mode, type, selection) {
	...
	if (status == 0) {
	var chat = "#fn맑은고딕#에이플러스 아이템 저장기능입니다. 아이템 저장기능이란 아이템이 초기화되거나 없어지는걸 방지해 아이템을 임의로 저장시켜 초기화나 아이템이 사라질경우 저장된시점의 아이템들을 복구받는 시스템입니다\r\n#L1##b지금바로 아이템을들 저장하겠습니다\r\n";
	cm.sendSimple(chat);
	} else if (status == 1) {
	...
	Itemlist();
	cm.sendOk("#fn맑은고딕##b모든 아이템들이 정상적으로 저장?습니다.");
	cm.dispose();return;
	} 
	} else if (status == 3) {
	 Itemlist ();
