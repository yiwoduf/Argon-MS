...
function action(mode, type, selection) {
	...
	if (cm.getPlayer().getMapId() == 105200200) {
      ...
      if(cm.getPlayer().getMapId() == 105200210) {
	if(cm.getMonsterCount(105200210) > 0) {
	cm.sendYesNo("서쪽 봉인 수호자 처치를 포기하겠습니까?");
		} else {
	cm.sendOk("루타비스로 이동시켜 드리겠습니다.");
		}
	} else {
	cm.sendSimple("#e#r<루타비스 서쪽 정원 입구>#n#k\r\n루타비스 서쪽 봉인의 수호자인 #r피에르#k가 지키고 있는 정원으로 가는 문이다.#r클리어 시점으로부터 24시간이 지나야 클리어 기록이 초기화 됩니다.\r\n\r\n#L0##b#i4033611##z4033611#를 사용하여 노말 모드로 이동한다. (100레벨 이상)#l\r\n#L1##i4033611##z4033611#를 사용하여 카오스 모드로 이동한다.(180레벨 이상)");
	}
    } else if (status == 1) {
	if(selection == 1) {
	...
	cm.dispose();
	return;
	} else if(cm.getPlayer().getMapId() == 105200210) {
	cm.warp(105200000);
	if(cm.getPlayer().getKeyValue("pier") != "clear") {
	cm.getPlayer().setKeyValue("pier","clear");
	cm.dispose();
	} else {
	cm.dispose();
	}
	...
	cm.sendOk("이미 다른 유저가 #r피에르#k를 처치하고 있는 중입니다.");
	cm.dispose();
	return;
	}
	if(cm.allPartyItem(4033611,1)) {
	cm.sendOk("#b오후의 티 테이블에#k 입장하기 위해서는 #i4033611##b#z4033611##k가 필요합니다.");
	cm.dispose();
	return;
	}
	cm.resetMap(105200200);
	cm.allPartyWarp(105200200,true);
