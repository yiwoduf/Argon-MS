...
function action(mode, type, selection) {
	...
	if(status == 0) {
	cm.sendSimple("#L0#노말아카이럼 소환하기#l#k\r\n#L1#마을가기#l#k\r\n\r\n#L2##r#e(보스소환이 안될시 클릭)#l#k\r\n");
	} else if (status == 1) {
	if(selection == 0) {
 if (cm.getMonsterCount(272020200) > 0) {
  cm.sendOk("모든 몬스터를 전멸시켜야 합니다.");
  cm.dispose();
} else { 
 cm.spawnMob(8860005,353,-181);
 cm.dispose();
}
	} else if (selection == 1) {
	...
	} else if (cm.getPlayer().getMapId() == 272020210) {
	if(status == 0) {
	cm.sendSimple("#L0#하드아카이럼 소환하기#l#k\r\n#L1#마을가기#l#k\r\n\r\n#L2##r#e(보스소환이 안될시 클릭)#l#k\r\n");
		} else if (status == 1) {
	if(selection == 0) {
 if (cm.getMonsterCount(272020210) > 0) {
  cm.sendOk("모든 몬스터를 전멸시켜야 합니다.");
  cm.dispose();
} else { 
 cm.spawnMob(8860000,353,-181);
 cm.dispose();
}
	} else if (selection == 1) {
	cm.allPartyWarp(100050001,true);
