...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
	       ...
	       var quan = ((cm.itemQuantity(4310016) - (cm.itemQuantity(4310016) % 80)) / 80);
	       var chat = "안녕하세요 #h0#님! 반지를 제작하시러 오셧나요?\r\n";
	       chat += "#r#e 주의 아이템창이 꽉찰시 단풍잎만 빠져나갈수 있으니 아이템창을 확인하고 거래해주세요.#k#n\r\n";
	       chat += "현재 #h0#님의 #i4310016##z4310016# 갯수는 #r#e"+ leaf +" #n#k개 입니다.\r\n"
	       chat += "#L1##i1112940##z1112940#(#r#e올텟30,공마10#k#n) 제작:#i4310016# 100개\r\n"
	       chat += "#L2##i1112940##z1112940#(#r#e올텟50,공마30#k#n) 제작:#i4310016# 200개\r\n"
	       chat += "#L3##i1112940##z1112940#(#r#e올텟70,공마50#k#n) 제작:#i4310016# 300개\r\n"
	       chat += "#L4##i1112940##z1112940#(#r#e올텟100,공마70#k#n) 제작:#i4310016# 500개\r\n"
	       cm.sendSimple(chat);
 } else if (selection == 1) {
	if (cm.haveItem(4310016 , 100)){
	var cosex1 = cm.getPlayer().getInventory(MapleInventoryType.EQUIP).getNumFreeSlot();
	if (cosex1 < 1) {
	cm.sendOk("장비창을 1칸이상 비워주세요.");
	cm.dispose();
	return;
	}
	    cm.gainItem(4310016 , -100);
            cm.gainSponserItem(1112400,'[마인드온라인]',30,10,0);
            cm.sendOk("제작완료 아이템창 확인해주세요.");
            cm.dispose(); 
        } else {
            cm.sendOk("재료가 부족하거나 장비창이 꽉찼음");
            cm.dispose();
        }
 } else if (selection == 2) {
	...
	return;
	    ...
	    cm.gainItem(4310016 , -200);
            cm.gainSponserItem(1112940,'[마인드온라인]',50,30,0);
            cm.sendOk("제작완료 아이템창 확인해주세요.");
            cm.dispose(); 
        } else {
            cm.sendOk("재료가 부족하거나 장비창이 꽉찼음");
	...
	return;
	    ...
	    cm.gainItem(4310016 , -300);
            cm.gainSponserItem(1112940,'[마인드온라인]',70,50,0);
            cm.sendOk("제작완료 아이템창 확인해주세요.");
            cm.dispose(); 
        } else {
            cm.sendOk("재료가 부족하거나 장비창이 꽉찼음");
	...
	return;
	    ...
	    cm.gainItem(4310016 , -500);
            cm.gainSponserItem(1112940,'[마인드온라인]',100,70,0);
            cm.sendOk("제작완료 아이템창 확인해주세요.");
            cm.dispose(); 
        } else {
            cm.sendOk("재료가 부족하거나 장비창이 꽉찼음");
