...
if (mode == -1) {
	...
	if (cm.getPlayer().getMapId() == 680000000) {
		var ask = "#fn나눔고딕 Extrabold##b아이템 강화#k 의 세계는 무궁무진 하답니다.\r\n\r\n";
		ask += "                                      #b초월 포인트#k : #r"+cm.getTSD()+" P#k\r\n";		
		ask += "#L1##d아이템 강화 를 이용하겠습니다.#k\r\n";
		ask += "#L2##d아이템 강화 시스템 에 대한 설명을 듣고싶습니다.#k"
		cm.sendSimple(ask);
	} else {
		cm.sendOk("#fn나눔고딕 Extrabold##r아이템 강화는 광장에서만 이용이 가능합니다.#k");
		cm.dispose();
	}
	} else if (status == 1){
		sel = selection;
		if (sel == 1){
		var ok = false;
		var option = "#fn나눔고딕 Extrabold##b강화할 아이템을 선택해 주세요.#k\r\n#d";
		...
		if (!ok) {
			cm.sendOk("#fn나눔고딕 Extrabold##r소지중인 장비 아이템이 없네요.#k");
			cm.dispose();
			return;
		}
		cm.sendSimple(option + "#k");
		} else if (sel == 2) {
			cm.sendOk("#fn나눔고딕 Extrabold##b초월 포인트#k 를 이용한 #d아이템 강화#k 는\r\n일반적인 주문서의 #d아이템 강화#k 와 다르게\r\n\r\n#b[힘, 민첩, 지능, 운]#k 의 #b기본 스탯#k 뿐만 아니라,\r\n#b[공격력, 방어력, 업그레이드 가능 횟수]\r\n[HP, MP, 회피율, 명중률, 스피드, 점프력]#k 등\r\n#b부가적인 스탯#k 을 큰 폭으로 #r총 3 가지#k 얻을 수 있습니다.\r\n\r\n#d아이템 강화#k 를 진행하기 위해서는\r\n#b초월 포인트#k #r"+cost+" P#k 가 필요합니다.");
			cm.dispose();
		}
	} else if (status == 2) {
		...
		if (statsSel == null) {
			cm.sendOk("#fn나눔고딕 Extrabold##r알 수 없는 에러입니다.\r\n처음부터 다시 시도해주세요.#k");
			cm.dispose();
			return;
		}
		var option = statsSel;
		var options = "#fn나눔고딕 Extrabold##r선택한 아이템#k\r\n\r\n#i" + option.getItemId() + "# #d(#t" + option.getItemId() + "#)#k\r\n\r\n#b해당 아이템의 현재 옵션#k#d\r\n\r\n";
		...
		options += "* 착용 가능 레벨 : " + option.getLevel() + "\r\n";
		options += "* 업그레이드 가능 횟수 : " + option.getUpgradeSlots() + "\r\n";
		options += "* 가위 사용 가능 횟수 : " + option.getFire() + "\r\n";
		options += "* 황금 망치 재련 여부 : " +vicious + "#k";
		cm.sendNext(options);	
	} else if (status == 3){
		...
		option.setFire(option.getFire() + 0);
		} else {
		option.setFire(20);
		}
		switch(chance){
		...
		switch(chanceb){
			...
			if (chance < 16){
				cm.sendOk("#fn나눔고딕 Extrabold##r강화가 적용된 아이템#k\r\n\r\n#i"+statsSel.getItemId()+"# #d(#t" + option.getItemId() + "#)#k\r\n\r\n#b강화된 옵션#k\r\n\r\n#d"+whatup+" : "+chance_select+"\r\n"+whatupa+" : "+chancea_select+"\r\n"+whatupb+" : "+chanceb_select+"#k");
				cm.getPlayer().dropMessage(5, "축하합니다.! 아이템 업그레이드가 성공 했습니다.!");
				cm.getPlayer().dropMessage(5, ""+whatup+"이(가) "+chance_select+"만큼 증가 했습니다.!");
				cm.getPlayer().dropMessage(5, ""+whatupa+"이(가) "+chancea_select+"만큼 증가 했습니다.!");
				cm.getPlayer().dropMessage(5, ""+whatupb+"이(가) "+chanceb_select+"만큼 증가 했습니다.!");
				cm.gainTSD(-cost);
				cm.dispose();
			} else if (chance >= 16){
				cm.sendOk("#fn나눔고딕 Extrabold##r아쉽게도 아이템의 강화에 실패하셨습니다.#k");
				cm.getPlayer().dropMessage(5, "#fn나눔고딕 Extrabold##r아이템 강화가 실패했습니다.#k");
				cm.gainTSD(-cost);
				cm.dispose();
			}
			} else {
