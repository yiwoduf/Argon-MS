...
function send(i, str)
{
	/*
		...
		30 : "보라"
	*/
	cm.getPlayer().send(Packages.packet.creators.MaplePacketCreator.getGMText(i, str));
}
...
function action(M, T, S)
{
	...
	if(St == 0)
	{
		...
		cm.sendSpirit("안녕하세요? 메이플월드를 여행하는 일은 즐거우신가요?\r\n"
			+ "#L0##r대화를 끝낸다.#l\r\n"
			+ "#L1##b아이템의 옵션을 변경한다.#l\r\n"
			+ "#L2##b잠재능력 코드를 확인한다.#l", true, 0);
	}
		...
		if(rotation != -1)
		{
                        ...
                        if (St ==4 && rotation == 2)
			{
				switch(S3)
				{
					...
					case 16: send(10, "스타포스 강화는 현재 준비중에 있습니다."); break;
...
function addItemInfo()
{
	if(rotation == 0)
	{
		...
		txt = "#r#e[아이템 기본 옵션]#n\r\n#b#fs11#";
		sel = ["힘", "덱스", "인트", "럭", "최대 체력(HP)", "최대 마나(MP)", "공격력", "마력", "물리방어력", "마법방어력", "명중률", "회피치", "스피드", "점프력", "주문서 성공 횟수", "업그레이드 가능 횟수", "스타포스 성공 횟수", "놀라운 장비강화 주문서 성공 횟수", "보스 공격 시 데미지", "몬스터 방어력 무시", "총 데미지", "올스탯", "착용 레벨 감소", "잠재능력 등급", "1 번째 잠재능력", "2 번째 잠재능력", "3 번째 잠재능력", "4 번째 잠재능력", "5 번째 잠재능력", "6 번째 잠재능력"];
