...
function action(mode, type, selection) {
    if (mode == -1) {
	    ...
	    chat += "#b#L100#"+k+" #e캐시샾#n으로 이동하고 싶습니다.#l\r\n";
	    chat += "#b#L9071003#"+k+" #e몬스터 파크#n를 이용하고 싶습니다.#l\r\n";
	    if(cm.getPlayer().getJob() >= 3300 && cm.getPlayer().getJob() <= 3312) {
	    chat += "#b#L2159314##e"+k+"재규어를  #n무료로 잡고싶습니다.#l\r\n";
	    }
	    chat += "#b#L9010022#"+k+" #e차원의 거울#n을 이용하고 싶습니다.#l\r\n";
	    chat += "#b#L1200#"+k+" 이 곳을 떠나 #e다른 곳#n으로 가고 싶습니다.#l\r\n";
	    chat += "#b#L101000100#"+k+" #e마인드온라인 작방#n으로 이동하고 싶습니다.#l\r\n";
	    cm.sendSimple(chat);


		
        } else if (status == 1) {
	if(selection == 2159314) {
		cm.dispose();
		cm.openNpc(selection);
	} else if (selection == 100) {
		cm.EnterCS();
		cm.dispose();
	} else if (selection == 101000100) {
		cm.warp(selection);
		cm.dispose();
	} else if(selection == 120020000) {
		cm.warp(selection);
		cm.dispose();
	} else if (selection == 9071003) {
	cm.dispose();
	cm.openNpc(selection);
	} else if (selection == 9010022) {
	cm.dispose();
	cm.openNpc(selection);
	...
	//warpmain += "\r\n#L8#"+k+" #r인내심테스트#n#Cgray##b를 하고 싶습니다.";
	...
	var hunt = "#b#h ##k님 환영합니다~♬ 인내심테스트를 즐겨보세요!\r\n\r\n"
	hunt += "#r#e[인내심 테스트]\r\n#k#n";
	hunt += "#L00##b인내심테스트#l";
	cm.sendSimple(hunt);
	...
	town += "#L914040000##b퀸스로드 　　　　#k │ #b에레브　　　#k │ #b에레브\r\n";
