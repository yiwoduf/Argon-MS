...
goods = [[1152108, 1, 500000], [1152110, 1, 500000], [1152111, 1, 500000], [1152112, 1, 500000], [1152112, 1, 500000], [1102275, 1, 500000], [1102276, 1, 500000], [1102277, 1, 500000], [1102278, 1, 500000], [1102279, 1, 500000], [4310153, 1000, 100000], [2049704, 3, 100000], [1092056, 1, 100000], [1342069, 1, 100000]];
	...
	else if(St == 1)
	{
		...
		switch(S0)
		{
			...
			case 2: strz = new StringBuilder();
				...
				switch(S1)
				{
					case 0: rank = con.prepareStatement("SELECT * FROM bossraidtime where diff = '쉬움' ORDER BY time ASC LIMIT 100").executeQuery(); break;
					case 1: rank = con.prepareStatement("SELECT * FROM bossraidtime where diff = '보통' ORDER BY time ASC LIMIT 100").executeQuery(); break;
					case 2: rank = con.prepareStatement("SELECT * FROM bossraidtime where diff = '어려움' ORDER BY time ASC LIMIT 100").executeQuery(); break;
					case 3: rank = con.prepareStatement("SELECT * FROM bossraidtime where diff = '매우 어려움' ORDER BY time ASC LIMIT 100").executeQuery(); break;
				}
				while(rank.next())
				{
					getRaidRankigByTime();
...
{
	con = MYSQL.getConnection();
	rank = con.prepareStatement("SELECT * FROM bossraidtime WHERE id = ?");
	rank.setString(1, i);
...
function getRaidRankigByTime()
{
	...
	if(nums < 10)
	{
		strz.append("#Cgray#").append(Integer.parseInt(0)+0).append("#b#e").append(Integer.parseInt(nums))
	}
	else if(nums >= 10 && nums < 100)
	{
		strz.append("#Cgray#").append(Integer.parseInt(0)).append("#b#e").append(Integer.parseInt(nums))
	}
	else
	{
	...
	strz.append(rank.getString("name")).append("#k (#d파티원 #e").append(Integer.parseInt(rank.getInt("size"))).append("#n명#k, #r체력 #e").append(Integer.parseInt(rank.getInt("rate"))).append("#n배#k)\r\n")
}
