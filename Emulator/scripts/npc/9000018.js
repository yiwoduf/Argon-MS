...
function start() {
    createAccount();
    status = -1;
    cm.sendSimple("#fn나눔고딕 Extrabold# 은하수 은행입니다. 무엇을 하시겠습니까?.\r\n증발의 위험이 있으니 넣으시고 시간 보이게끔해서 캡처부탁드립니다^^\r\n#r(#h #님의 통장잔액 : #d"+getAccount(cm.getPlayer().getName(),1)+"#r 메소)\r\n#b#L1#입금 #L2#출금 #L3#송금 #L10#이자 #e#r#L5#초기화");
}
...
function action(mode,type,selection) {
    ...
    if (status == 0) {
        if (selection == 1) {
            cm.sendGetNumber("#fn나눔고딕 Extrabold# 입금할 액수를 입력해주세요. (한 번 입금 최대 액수 :: 20억)",1,1,2000000000);
        } else if (selection == 2) {
            cm.sendGetNumber("#fn나눔고딕 Extrabold# 출금할 액수를 입력해주세요. (한 번 출금 최대 액수 :: 20억)",1,1,2000000000);
        } else if (selection == 3) {
            cm.sendGetText("#fn나눔고딕 Extrabold# 누구에게 보내시겠습니까?\r\n#r(주의 : 은행에 해당 이름이 없을 시에는 송금이 불가능합니다.)");
        } else if (selection == 5) {
            cm.sendYesNo("#fn나눔고딕 Extrabold# 정말 초기화하겠습니까? 계속 진행하시려면 예를 눌러주세요.");
	...
	var day = Math.floor(time / (60 * 60 * 24));
        sec = Math.floor(sec);
            if (time == 0) {
                ...
                text += "#fn나눔고딕 Extrabold# 이자는 #r5일, 7일, 11일, 15일, 30일#k 간격으로 받으실 수 있으며, 기간이 길수록 늘어나는 양도 커집니다. 그러나, 한 번 받으시면 다시 입금을 하셔야되니, 이 점 유의해주시기 바랍니다.\r\n";
        ...
        if (selected == 1) {
            money = selection;
            if (cm.getMeso() >= money) {
                cm.sendYesNo("#fn나눔고딕 Extrabold# 정말 #b"+money+" 메소#k를 입금하시겠습니까?");
            } else {
                cm.sendOk("#fn나눔고딕 Extrabold# 보유하신 메소보다 높은 액수를 입력하셨습니다.\r\n다시 한 번 입력해주세요.");
                cm.dispose();
            }
        } else if (selected == 2) {
            money = selection;
            if (getAccount(cm.getPlayer().getName(),1) >= money) {
                cm.sendYesNo("#fn나눔고딕 Extrabold# 정말 #b"+money+" 메소#k를 출금하시겠습니까?");
            } else {
                cm.sendOk("#fn나눔고딕 Extrabold# 보유하신 잔액보다 높은 액수를 입력하셨습니다.\r\n다시 한 번 입력해주세요.");
                cm.dispose();
            }
        } else if (selected == 3) {
            name = cm.getText();
            ...
            del.executeUpdate();
            sendHint("#fn나눔고딕 Extrabold# 은행 계정이 삭제되었습니다.",275,20);
            cm.dispose();
        } else if (selected == 10) {
            if (selection == 100) {
                cm.dispose();
                ...
                if (interest < 5) {
                    ...
                    cm.dispose();
                     ...
                     var benefit = interest == 5 && interest == 6 ? 0.05 :
                            ...
                            interest >= 30 ? 0.35 : 0; 
                    cm.sendYesNo("#fn나눔고딕 Extrabold# 현재 #h #님의 통장 잔액은 #b"+getAccount(cm.getPlayer().getName(),1)+" 메소#k이며, 추가 이자로#r"+getAccount(cm.getPlayer().getName(),1) * benefit+" ("+benefit+"%) 메소#k를 받게 됩니다. 받으면 시간 기록이 초기화되니 주의해주세요. 이자를 받으시겠습니까?");
                }
        ...
        if (selected == 1) {
            ...
            if (getAccount(cm.getPlayer().getName(),1) >= send) {
                ...
                sendHint("#fn나눔고딕 Extrabold# 보유하신 잔액보다 높은 액수를 입력하셨습니다.\r\n다시 한 번 입력해주세요.",300,30);
            }
        } else if (selected == 10) {
...
function updateAccount(name,money,date,time,mode) {
    ...
    upd.executeUpdate();
    cm.gainMeso(-money);
    sendHint("#fn나눔고딕 Extrabold# 성공적으로 입금하였습니다.",250,20);
    cm.dispose();
    } else {
    ...
    get.executeUpdate();
    cm.gainMeso(money);
    sendHint("#fn나눔고딕 Extrabold# 성공적으로 출금하였습니다.",250,20);
    cm.dispose();
} else if (mode == 3) {
    var ps = MYSQL.getConnection().prepareStatement("SELECT * FROM banksystem WHERE name = ?");
    ps.setString(1,name);
    ...
    mine.executeUpdate();
    sendHint("#fn나눔고딕 Extrabold# 성공적으로 송금하였습니다. (To. "+name+")",275,20);
    cm.dispose();
} else if (mode == 4) {
    var pse = MYSQL.getConnection().prepareStatement("SELECT * FROM banksystem WHERE name = ?");
    pse.setString(1,cm.getPlayer().getName());
    ...
    mine.executeUpdate();
    sendHint("#fn나눔고딕 Extrabold# 성공적으로 이자를 받았습니다. ("+money+" 메소)\r\n시간 기록이 초기화됩니다.",300,30);
    cm.dispose();
}
}
...
function checkName(name) {
    if (name == cm.getPlayer().getName()) {
        sendHint("#fn나눔고딕 Extrabold# 자신에게는 보낼 수 없습니다.",250,20);
        cm.dispose();
        return;
    }
    var ps = MYSQL.getConnection().prepareStatement("SELECT * FROM banksystem WHERE name = ?");
    ...
    if (isExist) {
        cm.sendGetNumber("#fn나눔고딕 Extrabold# 얼마를 보내시겠습니까? (최대 20억 메소 가능)\r\n#r(주의 : 송금을 하면 그만큼 자신의 통장잔고에서 차감됩니다.)",1,1,2000000000);
    } else {
