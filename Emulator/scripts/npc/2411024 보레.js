...
var sel;
var sel2;
var itemList = Array (2450042,2450054,2434661,2434654,2434529,2434655,2433981,3700222,3015276,3015437,3015438); // 아이템 목록 
...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		if (selection == 1) {
			cm.sendSpirit("으휴!!\r\n이래뵈도 나 바쁜 몸이라구!!\r\n용무가 없으면 대화좀 걸지말아 줄래?",false,2411024);
			cm.dispose();
		} else {
		...
		if (status2 == 1) {
			if (selection == 4) {
				...
				if (cm.getPlayer().getParty() == null) {
					cm.sendSpirit("\r\n너 설마 파티도 없이 입장할려고 하는거니?\r\n보스 레이드는 너가 생각 하는것 만큼 만만한곳이 아니라구!!",false,2411024);
					cm.dispose();
					return;
				}
				...
				if (!cm.allMembersHere()) {
					cm.sendSpirit("\r\n파티원 전원이 모이지도 않았는데 입장을 할려는 거야?",false,2411024);
					cm.dispose();
					return;
				}
				...
				if (!cm.BossCheck("rade",10)) {
					cm.sendSpirit("\r\n파티원중 누군가가 오늘 하루 입장 횟수를 초과했어.\r\n#r참고로 보스레이드 하루 이용 횟수는 10회로 제한되어있어",false,0);
					cm.dispose();
					return;
				}
		...
		if (selection == 0) {
			if (cm.getPlayer().getboss() >= number[sel]) {
				...
				cm.sendSpirit("너, 설마\r\n포인트도 없으면서 아이템을 교환할려고 한거니?\r\n포인트를 확실히 모아온 후에 다시 찾아오도록 해!!",false,2411024);
				cm.dispose();
			}
		} else if (selection == 1) {
