...
var 음치 = Array (2049301,2049401,1662000,1662001,1672000,1662002,1662003,1672022);
var 음치갯수 = Array (15,15,30,30,30,50,50,50);
...
var 박치 = Array (5064000,5064100,5064300);
var 박치갯수 = Array(10,10,10);

var 음치가격 = 2000; //메소로 교환할때의 가격
var 몸치가격 = 5000; //메소로 교환할때의 가격
var 박치가격 = 5000; //메소로 교환할때의 가격
...
function action(mode, type, selection) {
	...
	if (selection == 0) {
		...
		jessica2 += "#L0##i3010432# 낚시 의자 (5.000.000)\r\n";
		...
		if (sel == 1) {
			if (selection == 0) {
				if (cm.getMeso() >= 3000000) {
					...
					cm.sendOk("500만 메소가 있는게 확실한가? 음..다음에 다시 찾아와주게나" );
					cm.dispose();
				}
     			} else if (selection == 1) {
              	 		 if (cm.getMeso() >= 300000) {
               				 ...
               				 cm.dispose();
             	 	     	   ...
             	 	     	   cm.sendOk("30만 메소가 있는게 확실한가? 음..다음에 다시 찾아와주게나" );
				}
			}
		} else {
