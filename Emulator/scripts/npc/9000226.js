 ...
 */
var list1 = [1051486,1050417,1702668,1004701,1022244,1012517,1004528,1102766,1102832,1102861,1004603,1702620,1004403,1004569,1702607,1702608,1050370,1073075,1702595,1102836,1004541,1702594,1102837,1052948,1004540,1702538,1702534,1702565,1702535,1022230,1004181,1052762,1702525,1004167,1102699,1702575,1004472,1102824,1702599,1102841,1102822,1102250,1702597,1102844]; // 
var list2 = [1051391,1081013,1071077,1050321,1080007,1070060,1702504]; // 

var list3 = [1002186,1012289,1032024,1022079,1072153,1082102,1102039]; // 투명세트

var list4 = [1702291,1702361,1702365,1702364,1702155,1702196,1702211,1702400,1702366,1702398,1702416,1702397,1702424,1702444,1702443,1702340,1702466,1702453,1702457,1702464,1702473,1702470,1702476,1702458,1702486,1702487,1702488,1702485,1702497,1702455,1702405,1702469,1702503,1702502,1702512,1702480,1702521]; // 로얄 레인보우
var list5_1 = [1004029,1003492,1003493,1003494,1003495,1003496,1003131,1003132,1003133,1003846,1003847,1003860,1003873,1003874,1003875,1003876,1003877,1003878,1004034,1003542,1003543,1003544,1003545,1003546,1003547,1004026,1004027,1004028,1003807,1003954,1003901, 1004120,1004164,1004165,1004166,1004167,1004168,1004169,1004170,1004171,1004175,1004176,1003836,1003837,1003838,1003839]; // 기사단장 1세트 + 1004177
var list5_2 = [1112135,1112141,1112142,1112144,1112146,1112148,1112150,1112149.1112151,1112154,1112145,1112155,1112151,1112238,1112252,1112253,1112256,1112258,1112259,1112262,1112261,1112266,1112257,1112267,1112263]; // 기사단장 2세트
var list6 = [1042292,1042238,1042240,1042265,1042260,1042287,1042267,1042268,1042269,1042271,1042272,1042278,1042285,1042286,1042290,1042279,1042280,1042281,1042282]; // 시그너스 세트
var list7 = [1062174,1062185,1062171,1062172,1062174,1062176,1062179]; // 비커밍 팬텀 세트
var list8 = [1102546,1102547,1102548,1102376,1102377,1102378,1102307,1102308,1102309,1102466,1102572,1102624,1102630,1102450,1102451,1102643,1102511,1102629,1102615,1102625, 1102675,1102250,1102695,1102694,1102700,1102644,1102674,1003836,1003837,1003838,1003839]; // 윈터 스노우 세트
var list9 = [1702180,1702188,1702313,1702314,1702315,1702316]; // 엘븐 스피릿
var list10 = [1004180,1042319,1062207,1082588,1072934,1702523]; // 마린 보이 세트
var list11 = [1000069,1001092,1050311,1051383,1072901,1102667]; // 로얄 네이비
...
function action(mode,type,selection) {
	...
	if (selection % 100 != 0) {
	sendHint("정해진 후원포인트의 단위에 맞추어 적어주세요.\r\n#r※ 100 후원포인트#k = #b올스텟 ＋20#k",270,30);
	cm.dispose();
	} else {
	cost += selection;
	allstat = selection/100 * 20;
	allstatprice = selection;
	cm.sendGetNumber("\r\n#e선택 아이템#n : #i"+itemid+"# #z"+itemid+"#\r\n#e아이템 가격#n : #b#e"+cost+"#n 후원포인트#k\r\n\r\n#e#Cgray#※ "+allstatprice+" 후원포인트 = 올스텟 ＋"+allstat+" #k#n\r\n#r※ 100 후원포인트#k = #b공격력/마력 ＋2#k",0,0,cm.getPlayer().getRC());
}
}
	} else if (status == 3) {
	if (selected == 999999) {
	if (selection % 100 != 0) {
	sendHint("정해진 후원포인트의 단위에 맞추어 적어주세요.\r\n#r※ 100 후원포인트#k = #b올스텟 ＋5#k",270,30);
	cm.dispose();
	} else {
	cost += selection;
	allstat = selection/100 * 5;
	allstatprice = selection;
	cm.sendGetNumber("\r\n#e선택 아이템#n : #i"+itemid+"# #z"+itemid+"#\r\n#e아이템 가격#n : #b#e"+cost+"#n 후원포인트#k\r\n\r\n#e#Cgray#※ "+allstatprice+" 후원포인트 = 올스텟 ＋"+allstat+" #k#n\r\n#r※ 100 후원포인트#k = #b공격력/마력 ＋2#k",0,0,cm.getPlayer().getRC());
}
	} else {
	if (selection % 1000 != 0) {
	sendHint("정해진 후원포인트의 단위에 맞추어 적어주세요.\r\n#r※ 100 후원포인트#k = #b공격력/마력 ＋2#k",270,30);
	cm.dispose();
	} else {
	cost += selection;
	atk = selection/1000 * 30;
	atkprice = selection;
...
function showInfo() {
	...
	text += "　◆ 아이템 추가 문의는 #r루돌이#k or #b카톡qweasd1616#k 에게 문의해주세요.\r\n";
        text += "\r\n";
