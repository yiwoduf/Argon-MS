...
function action(mode, type, selection) {
	...
	sL = selection;
	if (sL == 3) {
	var text = cm.getScrollList();
	if (text != "") {	
		cm.sendSimple("분해할 주문서를 선택해 보시죠\r\n" + text);
	} else {
		cm.sendOk("분해할 주문서가 존재하지 않습니다.");
		cm.dispose();
		return;
	}
	} else {
	...
	if (sL == 3) {
		if (cm.canHold(4001832)) {
			...
			cm.sendOk("#b#z" + selection + "##k(을)를 분해하여 #b#z" + RdrN + "##k을 " + rand + "개 획득 하였습니다.");
			cm.gainItem(4001832,rand);
			cm.gainItem(selection, -1);
			cm.dispose();
		} else {
			cm.sendOk("소비 탭의 공간이 부족한 것 같군요… 한 칸 이상의 여유 공간을 확보한 뒤 다시 시도해보시기 바랍니다…");
			cm.dispose();
		}
	} else {
