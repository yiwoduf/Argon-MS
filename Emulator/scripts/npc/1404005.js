...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        chat += "#b#L1##e코어 젬스톤을 사용해 보스공격력을 강화한다.#k#l\r\n";        
        cm.sendSimple(chat);
    } else if (status == 1) {
        slot = selection;
        sel = selection == 0 ? "올스텟" : "공격력/마력";
        var itemid = 0;
        text = "#b#e보공강화#n#k를 선택 하셧습니다.\r\n강화할 아이템을 선택 해 주세요.\r\n";
        for (i = 0; i < 101; i++) {
            if (cm.getEquip(i)) {
        ...
        cm.sendYesNo("선택한 스탯 : 보스공격력\r\n다음 강화로 올라갈때 실패확률은 없으며\r\n수치는 보공 +5% 씩 추가됩니다.\r\n강화를 진행 하시겠습니까?");
    } else if (status == 3) {
        cm.gainItem(2435719, -1);
        eq.setBossDamage(eq.getBossDamage() + 5);
	cm.fakeRelog();
	cm.updateChar();
	cm.sendOk("강화에 성공하여 보공 5%가 추가되었습니다."); 
        cm.dispose();
    }
}