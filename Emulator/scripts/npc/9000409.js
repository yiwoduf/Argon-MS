	...
	if (status == 0) {
        ...
        if (true) {
	       if (select == 0) {
                    ...
                    var map = cm.getClient().getChannelServer().getMapFactory().getMap(109050000);
                    while (it.hasNext()) {
                        var chr = it.next();
                        if (!checkPosO(chr.getPosition()) && !chr.isGM()) {
                            chr.Message("안타깝네요! 정답은 O 였어요!  OX 퀴즈에서 퇴출되셨습니다. 다음 이벤트에 도전해 주세요!");
                            chr.changeMap(map, map.getPortal(0));
                            cm.getPlayer().getMap().broadcastMessage(Packages.packet.creators.MainPacketCreator.getGMText(10, chr.getName() + "님이 OX퀴즈에서 퇴장당했습니다!"));
                        }
                    }
                    cm.dispose();
                } else if (select == 3) {
