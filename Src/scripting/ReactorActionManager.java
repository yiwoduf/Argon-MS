    ...
    public void gainGuildPot() {
        ...
        try {
                ...
                MapTimer.getInstance().schedule(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            PreparedStatement ps = con.prepareStatement("DELETE FROM `pots` WHERE `gid` = ?");
            ...
            if (reactor.getGuildid() != getPlayer().getGuildId()) {
                // getPlayer().getClient().getSession().writeAndFlush(MaplePacketCreator.getNPCTalk(reactor.getReactorId(), (byte) 0, "저는 #b" + reactor.getName() + "길드#k의 나무입니다.\r\n안타깝게도 당신은 나에게 물을 줄 수 없어요,", "00 00", (byte) 0));
