...
public class AbstractPlayerInteraction {
    ...
    public final boolean canHold(final int itemid, final int quanatity) {
        return canHold(itemid);
    }
	...
	c.getSession().writeAndFlush(MainPacketCreator.getShowItemGain(id, quantity, true));
    }
	...
	c.getSession().writeAndFlush(MainPacketCreator.serverNotice(type, message));
    }
	...
	for (final MapleCharacter chr : party) {
		...
		chr.getClient().getSession().writeAndFlush(MainPacketCreator.getShowItemGain(id, (short) -possesed, true));
	    }
	}
    }
...
//	c.getSession().writeAndFlush(UIPacket.getStatusMsg(id));
    ...
    public final int getCTS_MorphState() {
	return c.getPlayer().getCTS_MorphState();
    }
	...
	if (possessed > 0) {
	    ...
	    c.getSession().writeAndFlush(MainPacketCreator.getShowItemGain(id, (short) -possessed, true));
	}
    }
	...
	if (getPlayer().getPet(index) != null) {
	    ...
	    getClient().getSession().writeAndFlush(PetPacket.updatePet(getPlayer(), getPlayer().getPet(index), false, getPlayer().getPetLoot()));
	}
    }
	...
	for (final MaplePet pet : getPlayer().getPets()) {
		...
		getClient().getSession().writeAndFlush(PetPacket.updatePet(getPlayer(), pet, false, getPlayer().getPetLoot()));
	    }
	}
    }
	...
	c.getSession().writeAndFlush(UIPacket.showWZEffect(data, value));
    }
    ...
    public void spawnNPCTemp(final int id, final int x, final int y) {
        ...
        getClient().getSession().writeAndFlush(MainPacketCreator.spawnNPCTemp(x,y));
    }
    ...
    public void addEquip2(short pos, int itemid) {
        ...
        equip.addFromDB(eq.copy());
        if (isExist)
            c.getSession().writeAndFlush(MainPacketCreator.updateSpecialItemUse(eq, eq.getType()));
        else {
            fakeRelog();
            updateChar();
        }
    ...
    public final void warningStartBoss(String name) {
        WorldBroadcasting.broadcastGM(MainPacketCreator.getGMText(5, "[GM알림] "+getClient().getChannel()+" 채널서버에서 "+name+" 보스 레이드가 시작되었습니다. 맵 코드 : "+getPlayer().getMapId()));
    }
    ...
    public final void Auction() {
        c.getSession().writeAndFlush(MainPacketCreator.getAuctionInfo());
    }
