...
public class EventInstanceManager {
	...
	try {
	    for (final MapleCharacter chr : chars) {
		chr.getClient().getSession().writeAndFlush(UIPacket.AchievementRatio(achivementRatio));
	    }
	} finally {
	    mutex.unlock();
	}
	...
	try {
	    for (final MapleCharacter chr : chars) {
		chr.getClient().getSession().writeAndFlush(MainPacketCreator.serverNotice(type, msg));
	    }
	} finally {
	    mutex.unlock();
	}
    ...
    public final void broadcastPacket(final byte[] packet) {
	mutex.lock();
	try {
	    for (final MapleCharacter chr : chars) {
