...
public class InventoryManipulator {
    ...
    public static void addRing(MapleCharacter chr, int itemId, int ringId, int sn, String partner) {
        ...
        //       return;
        //    }
        //     chr.getClient().getSession().writeAndFlush(MainPacketCreator.addInventorySlot(type, nEquip));
        //   chr.getClient().getSession().writeAndFlush(CashPacket.sendBoughtRings(GameConstants.isCrushRing(itemId), ring, sn, chr.getClient().getAccID(), partner));
    }
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!GameConstants.isThrowingStar(item.getItemId()) && !GameConstants.isBullet(item.getItemId())) {
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax && item.getOwner().equals(eItem.getOwner()) && item.getExpiration() == eItem.getExpiration()) {
                                ...
                                eItem.setQuantity(newQ);
                                if (!c.isCS) {
                                    c.getSession().writeAndFlush(MainPacketCreator.updateInventorySlot(type, eItem, petloot ? false : true));
                                }
                                if (eItem.isCash()) {
                ...
                while (quantity > 0) {
                    ...
                    if (!c.isCS) {
                        c.getSession().writeAndFlush(MainPacketCreator.addInventorySlot(type, nItem, petloot ? false : true));
                    }
                }
            } else {
                // Throwing Stars and Bullets - Add all into one slot regardless of quantity.
            ...
            if (quantity == 1) {
                ...
                if (!c.isCS) {
                    c.getSession().writeAndFlush(MainPacketCreator.addInventorySlot(type, item, petloot ? false : true));
                }
            } else {
    ...
    public static void move(final MapleClient c, final MapleInventoryType type, final short src, final short dst) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP) && initialTarget != null
            ...
            if ((olddstQ + oldsrcQ) > slotMax) {
                ...
                c.getSession().writeAndFlush(MainPacketCreator.moveAndMergeInventoryItem(type, src, dst, ((Item) c.getPlayer().getInventory(type).getItem(dst)).getQuantity(), bag, switchSrcDst, bothBag));
            }
        } else {
            c.getSession().writeAndFlush(MainPacketCreator.moveInventoryItem(type, switchSrcDst ? dst : src, switchSrcDst ? src : dst, eqIndicator, bag, bothBag));
        }
    }
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (GameConstants.isWeapon(source.getItemId())) {
            c.getPlayer().cancelEffectFromBuffStat(BuffStats.CTS_Booster, -1);
            c.getPlayer().cancelEffectFromBuffStat(BuffStats.CTS_NoBulletConsume, -1);
            c.getPlayer().cancelEffectFromBuffStat(BuffStats.CTS_SoulArrow, -1);
            c.getPlayer().cancelEffectFromBuffStat(BuffStats.CTS_WeaponCharge, -1);
        }
        if (source.getItemId() >= 1113098 && source.getItemId() <= 1113122) {
            c.getPlayer().changeSkillLevel(source.getItemId() + 78888357, (byte) 4, (byte) 4);
        }
        c.getSession().writeAndFlush(MainPacketCreator.moveInventoryItem(MapleInventoryType.EQUIP, src, dst, (byte) 2, false, false));
        if (source.getState() > 0) {
            ...
            for (int i : potentials) {
                ...
                if (pot.skillID > 0 && chr.getSkillLevel(PlayerStats.getSkillByJob(pot.skillID, c.getPlayer().getJob())) <= 0) {
                    chr.changeSkillLevel(SkillFactory.getSkill(PlayerStats.getSkillByJob(pot.skillID, chr.getJob())), (byte) 1, (byte) 1);
                }
            }
        }
        chr.equipChanged();
        ...
        if (stats.get("equipTradeBlock") == 1 || source.getItemId() / 10000 == 167 || source.getFire() >= 0) { // Block trade when equipped.
            ...
            if (!ItemFlag.UNTRADEABLE.check(flag)) {
                ...
                source.setFlag(flag);
                c.getSession().writeAndFlush(MainPacketCreator.updateSpecialItemUse(source, GameConstants.getInventoryType(source.getItemId()).getType()));
            }
        }
        if (source.getItemId() / 10000 == 167) { //Heart
            ...
            if (android != null) {
                ...
                if (ItemFlag.ANDROID_ACTIVATED.check(flag)) {
                    ...
                    c.getSession().writeAndFlush(MainPacketCreator.updateSpecialItemUse(android, GameConstants.getInventoryType(android.getItemId()).getType()));
                }
            }
        }
    ...
    public static void unequip(final MapleClient c, final short src, final short dst) {
        ...
        if (source.getState() > 0) {
            ...
            for (int i : potentials) {
                ...
                StructPotentialItem pot = ii.getPotentialInfo(i).get((ii.getReqLevel(source.getItemId()) / 10) - 1);
                if (pot.skillID > 0 && c.getPlayer().getSkillLevel(PlayerStats.getSkillByJob(pot.skillID, c.getPlayer().getJob())) > 0) {
                    c.getPlayer().changeSkillLevel(SkillFactory.getSkill(PlayerStats.getSkillByJob(pot.skillID, c.getPlayer().getJob())), (byte) 0, (byte) 0);
                }
            }
        }
        if (c.getPlayer().getBuffedValue(BuffStats.CTS_Booster) != null && GameConstants.isWeapon(source.getItemId())) {
            c.getPlayer().cancelBuffStats(-1, BuffStats.CTS_Booster);
        }
        ...
        c.getSession().writeAndFlush(MainPacketCreator.moveInventoryItem(MapleInventoryType.EQUIP, src, dst, (byte) 1, false, false));
    ...
    public static void drop(final MapleClient c, MapleInventoryType type, final short src, final short quantity) {
        ...
        if (quantity < source.getQuantity() && !GameConstants.isThrowingStar(source.getItemId()) && !GameConstants.isBullet(source.getItemId())) {
            ...
            c.getSession().writeAndFlush(MainPacketCreator.dropInventoryItem((src < 0 ? MapleInventoryType.EQUIP : type), src));
            if (src < 0) {
                c.getPlayer().equipChanged();
            }
