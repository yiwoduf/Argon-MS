...
public class MapleStorage {
    ...
    public void send2ndPWChecking(MapleClient c, int npcId, boolean compose) {
        lastNPC = npcId;
        iscompose = compose;
        c.send(MainPacketCreator.get2ndPWChecking(true));
    }
    ...
    public void sendStorage(MapleClient c) {
        if (!iscompose) { // Celphis
            ...
            c.send(MainPacketCreator.getStorage(lastNPC, slots, items, meso));
        } else {
            oldItems.addAll(items);
            items = new LinkedList<IItem>();
            c.send(MainPacketCreator.getStorage(lastNPC, (byte) 3, items, 0));
        }
    }

    public void update(MapleClient c) {
        c.send(MainPacketCreator.arrangeStorage(slots, items, true));
    }

    public void sendStored(MapleClient c, MapleInventoryType type) {
        c.send(MainPacketCreator.storeStorage(slots, type, typeItems.get(type)));
    }

    public void sendTakenOut(MapleClient c, MapleInventoryType type) {
        c.send(MainPacketCreator.takeOutStorage(slots, type, typeItems.get(type)));
    }
    ...
    public void sendMeso(MapleClient c) {
        c.send(MainPacketCreator.mesoStorage(slots, meso));
    }
