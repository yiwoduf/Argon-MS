...
public class MapleWorldMapProvider {
    ...
    private static final MapleDataProvider source = MapleDataProviderFactory.getDataProvider(new File("property/Wz/Map.wz"));
    private static final MapleData nameData = MapleDataProviderFactory.getDataProvider(new File("property/Wz/String.wz")).getData("Map.img");
...
//                     System.out.println("ID " + rs.getInt("id") + "Something Added..");
                ...
                try {
                    Connection con = MYSQL.getConnection();
                    PreparedStatement ps = con.prepareStatement("SELECT time FROM bosscooltime WHERE map = ? AND channel = ?");
                    ps.setInt(1, mapid);
                    ps.setInt(2, channel);
                    ResultSet rs = ps.executeQuery();
                    if (rs.next()) {
