...
public class Event_DojoAgent {
    ...
    public static boolean warpStartAgent(final MapleCharacter c, final boolean party) {
        ...
        for (int i = mapid; i < mapid + 15; i++) {
            ...
            if (map.getCharactersSize() == 0) {
                clearMap(map, false, c.getClient());
    ...
    public static boolean warpNextMap(final MapleCharacter c, final boolean fromResting) {
        ...
        final ChannelServer ch = c.getClient().getChannelServer();
        if (!fromResting) {
            clearMap(ch.getMapFactory().getMap(currentmap), true, c.getClient());
            c.modifyCSPoints(1, 40, true);
            c.getClient().getSession().write(MainPacketCreator.Mulung_Pts(10, 0));
        }
    ...
    private static void clearMap(final MapleMap map, final boolean check, MapleClient c) {
        if (check) {
            if (map.getCharactersSize() != 0) {
                return;
            }
    ...
    private static void spawnMonster(final MapleMap map, final int stage) {
        ...
        if (mobid != 0) {
            ...
            EventTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                    map.spawnMonsterWithEffect(MapleLifeProvider.getMonster(mobid), 15, rand == 0 ? point1 : rand == 1 ? point2 : point3);
                }
            }, 3000);
        }
    }
