...
public class MapleMiniGame extends AbstractPlayerStore {
    ...
    public MapleMiniGame(MapleCharacter owner, int ownerId, int ownerAccId, Point pos, int itemId, String desc, int createType, String pass) {
        super(owner, owner.getId(), owner.getAccountID(), owner.getPosition(), itemId, desc, slots - 1); //?
    ...
    public void checkWin() {
        if (getPoints() >= getMatchesToWin() && !isOpen()) {
            ...
            this.broadcastToVisitors(MiniGamePacket.getMiniGameResult(this, tie ? 1 : 2, x));
    ...
    public void send(MapleClient c) {
        ...
        c.send(MiniGamePacket.getMiniGame(c, this));
    }
    ...
    public void setPiece(int move1, int move2, int type, MapleCharacter chr) {
        if (piece[move1][move2] == 0 && !isOpen()) {
            ...
            this.broadcastToVisitors(MiniGamePacket.getMiniGameMoveOmok(move1, move2, type));
            boolean found = false;
            for (int y = 0; y < 15; y++) {
                for (int x = 0; x < 15; x++) {
                    if (!found && searchCombo(x, y, type)) {
                        this.broadcastToVisitors(MiniGamePacket.getMiniGameResult(this, 2, getVisitorSlot(chr)));
                        this.setOpen(true);
                        update();
                        checkExitAfterGame();
                        found = true;
    ...
    public byte getShopType() {
        return GameType == 1 ? (byte) 3 : (byte) 4;
    }
