...
public class HiredMerchant extends AbstractPlayerStore {
    ...
    public HiredMerchant(String ownerName, int ownerId, int ownerAccId, Point pos, int map, int channel, int itemId, String desc, long startTime) {
        super(ownerName, ownerId, ownerAccId, pos, itemId, desc, 6);
        start = startTime;
        this.map = ChannelServer.getInstance(channel).getMapFactory().getMap(map);
        this.channel = channel;
        WorldTimer.getInstance().schedule(new Runnable() {
    ...
    public void sendDestroyData(MapleClient client) {
        client.send(PlayerShopPacket.destroyHiredMerchant(getOwnerId()));
    }

    @Override
    public void sendSpawnData(MapleClient client) {
        client.send(PlayerShopPacket.spawnHiredMerchant(this));
    }
