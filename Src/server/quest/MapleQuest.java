...
public class MapleQuest {
    ...
    public static final MapleDataProvider questData = MapleDataProviderFactory.getDataProvider(new File("property/Wz/Quest.wz"));
    ...
    public static void cacheMapleQuest() {
        synchronized(questReady) {
            if (!questReady) {
                System.out.println("[ARGON] 퀘스트 데이터 캐싱을 시작합니다.");
               long t = System.currentTimeMillis();
                for (MapleData d : actions.getChildren()) {
                 ...
                 System.out.println("[ARGON] 퀘스트 데이터 캐싱이 완료되었습니다. 데이터 : " + quests.size() +" 소요시간 : "+(System.currentTimeMillis() - t)+"ms");
	...
	if (/*(autoPreComplete || checkNPCOnMap(c, npc)) && canComplete(c, npc)*/true) {
	   ...
	   c.getClient().getSession().writeAndFlush(MainPacketCreator.showSpecialEffect(14)); //1.2.251+, (+1)
	   c.getMap().broadcastMessage(c, MainPacketCreator.showSpecialEffect(c.getId(), 14), false); //1.2.251+, (+1)
	}
    }
