...
//        }
    ...
    public boolean purchaseSkill(int skill, String name, int cid, byte level) {
        final SkillStatEffect skillid = SkillFactory.getSkill(skill).getEffect(getSkillLevel(skill) + 1);
        if (skillid.getReqGuildLevel() > 1) {
            return false;
        }
        GuildSkills ourSkill = guildSkills.get(skill);
        if (ourSkill == null) {
            ourSkill = new GuildSkills(skill, skillid.getLevel() + 1, 0, name, name);
            guildSkills.put(skill, ourSkill);
        } else if (ourSkill != null && level >= 1) {
            ourSkill.level = getSkillLevel(skill) + 1;
            ourSkill.purchaser = name;
            ourSkill.activator = name;
        } else if (ourSkill != null && level <= 0) {
            if (getGP() > 49999) {
                ourSkill.level = getSkillLevel(skill) - 1;
                ourSkill.purchaser = name;
                ourSkill.activator = name;
    ...
    public final void saveGuildSkill(int guild) {
        try {
            Connection con = MYSQL.getConnection();
            PreparedStatement ps = con.prepareStatement("SELECT * FROM guilds WHERE guildid=" + guild);
            ResultSet rs = ps.executeQuery();
    ...
    public static final MapleGuildResponse sendInvite(final MapleClient c, final String targetName) {
        ...
        mc.getClient().getSession().writeAndFlush(MainPacketCreator.guildInvite(c.getPlayer().getGuildId(), c.getPlayer().getName(), c.getPlayer().getLevel(), c.getPlayer().getJob(), c.getPlayer().getId()));
        return null;
    }
