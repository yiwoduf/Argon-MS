...
public class TestMobSkill {
    ...
    public TestMobSkill(int skillId, int level) {
    ...
    public boolean checkCurrentBuff(MapleCharacter player, MapleMonster monster) { //스탯 수정 필.
        boolean stop = false;
        switch (skillId) {
            case 100:
            case 110:
                stop = monster.isTestBuffed(TestMonsterStatus.WEAPON_ATTACK_UP);
                break;
            case 101:
            case 111:
                stop = monster.isTestBuffed(TestMonsterStatus.MAGIC_ATTACK_UP);
                break;
            case 102:
            case 112:
                stop = monster.isTestBuffed(TestMonsterStatus.WEAPON_DEFENSE_UP);
                break;
            case 103:
            case 113:
                stop = monster.isTestBuffed(TestMonsterStatus.MAGIC_DEFENSE_UP);
                break;
            case 140:
            case 141:
            case 143:
            case 144:
            case 145:
                stop = monster.isTestBuffed(TestMonsterStatus.MAGIC_IMMUNITY) || monster.isTestBuffed(TestMonsterStatus.WEAPON_IMMUNITY);
                break;
            case 200:
    ...
    public void applyEffect(MapleCharacter player, MapleMonster monster, boolean skill) { //스탯 수정 필.
        ...
        switch (skillId) {
            case 100:
            case 110:
                if (monster.getBuff(TestMonsterStatus.MAGIC_CRASH) == null) {
                    stats.put(TestMonsterStatus.WEAPON_ATTACK_UP, Integer.valueOf(x));
                }
                break;
            case 101:
            case 111:
                if (monster.getBuff(TestMonsterStatus.MAGIC_CRASH) == null) {
                    stats.put(TestMonsterStatus.MAGIC_ATTACK_UP, Integer.valueOf(x));
                }
                break;
            case 102:
            case 112:
                if (monster.getBuff(TestMonsterStatus.MAGIC_CRASH) == null) {
                    stats.put(TestMonsterStatus.WEAPON_DEFENSE_UP, Integer.valueOf(x));
                    break;
                }
            case 103:
            case 113:
                if (monster.getBuff(TestMonsterStatus.MAGIC_CRASH) == null) {
                    stats.put(TestMonsterStatus.MAGIC_DEFENSE_UP, Integer.valueOf(x));
                }
                break;
            case 114:
            ...
            case 140:
                if (makeChanceResult()) {
                    stats.put(TestMonsterStatus.WEAPON_IMMUNITY, Integer.valueOf(x));
                }
                break;
            case 141:
                if (makeChanceResult()) {
            ...
            case 143: // Weapon Reflect
                stats.put(TestMonsterStatus.WEAPON_DAMAGE_REFLECT, Integer.valueOf(x));
                stats.put(TestMonsterStatus.WEAPON_IMMUNITY, Integer.valueOf(x));
                break;
        ...
        if (stats.size() > 0) {
            if (lt != null && rb != null && skill) {
                for (MapleMapObject mons : getObjectsInRange(monster, MapleMapObjectType.MONSTER)) {
                    ((MapleMonster) mons).applyMonsterTestBuff(stats, getX(), getSkillId(), getDuration(), this, reflection);
                }
            } else {
                monster.applyMonsterTestBuff(stats, getX(), getSkillId(), getDuration(), this, reflection);
            }
        }
        if (disease != null) {
            if (lt != null && rb != null && skill) {
                for (MapleCharacter chr : getPlayersInRange(monster, player)) {
                    chr.giveDeBuffTest(disease, this);
                }
            } else {
                player.giveDeBuffTest(disease, this);
            }
        }
