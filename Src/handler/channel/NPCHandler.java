...
public class NPCHandler {
    ...
    public static byte[] NPCAnimation(ReadingMaple slea, MapleClient c) {
    ...
    public static final void QuestAction(final ReadingMaple rh, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (action) {
            ...
            case 2: { // Complete Quest
                ...
                c.getSession().writeAndFlush(MainPacketCreator.completeQuest((short) quest));
                // 6 = start quest
                // 7 = unknown error
                // 8 = equip is full
                // 9 = not enough mesos
                ...
                c.getSession().writeAndFlush(MainPacketCreator.showSpecialEffect(14)); //1.2.239+, (+1) 
                chr.getMap().broadcastMessage(chr, MainPacketCreator.showSpecialEffect(chr.getId(), 14), false); //1.2.251+, (+1)
                break;
            }
            ...
            case 5: { // Scripted End Quest
                ...
                c.getSession().writeAndFlush(MainPacketCreator.showSpecialEffect(14)); //1.2.251+, (+1)
                chr.getMap().broadcastMessage(chr, MainPacketCreator.showSpecialEffect(chr.getId(), 14), false); //1.2.251+, (+1)
                break;
            }
        }
    ...
    //    c.getPlayer().Message("lastMsg : " + lastMsg + "");
        ...
        if (lastMsg == 3) {
        ...
        } else if (lastMsg == 0x29) {
            rh.skip(2);            
            int nMixBaseHairColor = rh.readInt();
            int nMixAddHairColor = rh.readInt();
            int nMixHairBaseProb = rh.readInt();
            rh.skip(13);
            c.getPlayer().dropMessage(6, rh.toString());
            if (!GameConstants.isZero(c.getPlayer().getJob())) {
                c.getPlayer().setMixBaseHairColor(nMixBaseHairColor);
                c.getPlayer().setMixAddHairColor(nMixAddHairColor);
                c.getPlayer().setMixHairBaseProb(nMixHairBaseProb);
                c.getSession().writeAndFlush(MainPacketCreator.setMix(c.getPlayer()));
                c.getSession().writeAndFlush(MainPacketCreator.resetActions());
                NPCScriptManager.getInstance().dispose(c);
            } else {
                c.getPlayer().setMixBaseHairColor(nMixBaseHairColor);
            ...
            } else if (rh.available() >= 2) {
                selection = rh.readByte();
                selection2 = rh.readByte();
            ...
            if (action != -1) {
                if (cm.getType() == 0) {
                    ...
                    NPCScriptManager.getInstance().action(c, action, lastMsg, selection, selection2);
                }
            } else {
                cm.dispose();
            }
