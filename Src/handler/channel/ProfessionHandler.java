...
//                    c.getPlayer().extendedslots_changed = true;
            c.getSession().writeAndFlush(MainPacketCreator.updateSpecialItemUse(toUse, (byte) 4, true, chr));
        }
        c.getSession().writeAndFlush(MainPacketCreator.openBag(chr.getExtendedSlots().indexOf(toUse.getUniqueId()), itemId, firstTime));
        c.getSession().writeAndFlush(MainPacketCreator.resetActions());
    }
