        ...
        if (pet != null) {
            ...
            if (chr.getPetIndex(pet) != -1) { //펫 해제
                ...
                for (Integer skill : petbufflist) {
                    chr.changeSkillLevel_NoSkip(SkillFactory.getSkill(80000000 + skill), (byte) -1, (byte) -1);
                }
            ...
            chr.send(PetPacket.showPet(chr, pet, false, false, false));
            chr.getMap().broadcastMessage(chr, PetPacket.showPet(chr, pet, false, false, true), false);
            chr.updatePet();
            ...
            if (!chr.getPetLoot()) {
                chr.setPetLoot(true);
                chr.send(PetPacket.updatePetLootStatus(1));
            }
            chr.ea();
        }
    }
        ...
        for (MaplePet pet : chr.getPets()) {
            ...
            if (pet.getFullness() < previousFullness) {
                ...
                if (pet.getFullness() < 100) {
                    ...
                    c.getSession().writeAndFlush(PetPacket.updatePet(chr, pet, false, chr.getPetLoot()));
                    chr.getMap().broadcastMessage(chr, PetPacket.commandResponse(chr.getId(), (byte) 1, (byte) (chr.getPetIndex(pet) + 1), false), true);
                }
